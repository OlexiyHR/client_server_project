...
public class WriteOffHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if ("GET".equals(exchange.getRequestMethod())) {
            byte[] response = Files.readAllBytes(Paths.get("src/pages/write-off.html"));
        ...
        if ("POST".equals(exchange.getRequestMethod())) {
            ...
            try {
                ...
                if (good != null) {
                    int old_amount = good.getAmount();
                    if (old_amount - amount >= 0) {
                        GoodDAO.writeOff(productName, amount);
                        String message = "Списано зі складу: " + productName + " - штук " + amount;
                        exchange.getResponseHeaders().set("Location", "/manage-quantity/" + URLEncoder.encode(message, StandardCharsets.UTF_8.toString()));
                        exchange.sendResponseHeaders(302, -1);
                    } else {
                        String errorMessage = "Помилка: Кількість товару " + productName + " = " + old_amount + ", а ви хочете списати " + amount;
