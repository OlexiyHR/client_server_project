...
public class SearchGoodHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if ("GET".equals(exchange.getRequestMethod())) {
            byte[] response = Files.readAllBytes(Paths.get("src/pages/search-good.html"));
        ...
        if ("POST".equals(exchange.getRequestMethod())) {
            ...
            Good good = GoodDAO.getGood(productName);
            if (good != null) {
                String response = generateResultPage(good);
                ...
                String response = generateErrorPage(productName);
                exchange.getResponseHeaders().set("Content-Type", "text/html; charset=UTF-8");
    ...
    private String generateResultPage(Good good) {
    ...
    private String generateErrorPage(String productName) {
        StringBuilder html = new StringBuilder();
        html.append("<!DOCTYPE html>");
        html.append("<html lang='ua'>");
        html.append("<head>");
        ...
        html.append("<h1>Результати пошуку</h1>");
        html.append("<div>");
        html.append("Немає товару з назвою  ").append(productName);
        html.append("</div>");
        html.append("<form method='GET' action='/menu'>");
        html.append("<button type='submit' class='btn'>Назад</button>");
        html.append("</form>");
