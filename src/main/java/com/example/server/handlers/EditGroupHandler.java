...
public class EditGroupHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if ("GET".equals(exchange.getRequestMethod())) {
            byte[] response = Files.readAllBytes(Paths.get("src/pages/edit-group.html"));
        ...
        if ("POST".equals(exchange.getRequestMethod())) {
            ...
            Group existingGroup = GroupDAO.getGroup(groupName);
            if (existingGroup != null) {

                if (!parameters.get("new_name").isEmpty()) {
                    existingGroup.setGroup_name(parameters.get("new_name"));
                }
                if (!parameters.get("description").isEmpty()) {
                    existingGroup.setDescription(parameters.get("description"));
                }
                if (GroupDAO.updateGroup(existingGroup, groupName)) {

                    String message = "Змінено групу: " + groupName;
                    exchange.getResponseHeaders().set("Location", "/manage-groups/" + URLEncoder.encode(message, StandardCharsets.UTF_8.toString()));
                    exchange.sendResponseHeaders(302, -1);
                } else {
                    String errorMessage = "Помилка: Група вже існує з іменнем " + parameters.get("new_name");
                    exchange.getResponseHeaders().set("Location", "/manage-groups/" + URLEncoder.encode(errorMessage, StandardCharsets.UTF_8.toString()));
                    exchange.sendResponseHeaders(302, -1);
                }

            } else {
                String errorMessage = "Помилка: Групу не знайдено з іменем " + groupName;
                exchange.getResponseHeaders().set("Location", "/manage-groups/" + URLEncoder.encode(errorMessage, StandardCharsets.UTF_8.toString()));
                exchange.sendResponseHeaders(302, -1);
            }
        }
